...
import {getWebsocketUrl, isDev, isValidUuid} from './utils';
  ...
  fetchLatestConversation = async (
    ...
    try {
      ...
      if (!customerId) {
        ...
        this.setCustomerId(null).setMessages([...this.getDefaultGreeting()]);
      ...
      if (!conversation) {
        ...
        // this.setState({messages: [...this.getDefaultGreeting()]});
        this.setCustomerId(customerId)
          .setMessages([...this.getDefaultGreeting()])
      ...
      this.setCustomerId(customerId)
        ...
        .setMessages([...this.getDefaultGreeting(), ...formattedMessages]);

      return conversation;

      // TODO: all the stuff below can be handled separately?
      // For the sake of this function, we just care about retrieving the initial
      // message(s) to display (whether that's the conversation history or just a greeting)
